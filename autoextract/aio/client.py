...
# async def request_parallel(urls: List[str],
#                            page_type: str,
#                            api_key: Optional[str] = None,
#                            *,
#                            endpoint: str = API_ENDPOINT,
...
#                            ):
#     results = []
#     it = iter_request_parallel(urls, page_type=page_type,
#                                api_key=api_key, endpoint=endpoint,
#                                session=session,
#                                batch_size=batch_size, n_conn=n_conn)
#     pbar = tqdm.tqdm(smoothing=0, leave=True, total=len(urls), miniters=1,
#                      unit="url", disable=not progress)
#     try:
#         async for res in it:
#             results.append(res)
#             pbar.update()
...
# async def iter_request_parallel(urls: List[str],
#                                 page_type: str,
#                                 api_key: Optional[str] = None,
#                                 *,
#                                 endpoint: str = API_ENDPOINT,
...
# FIXME: call await session.close()
#
#     async with session:
#         results_iter = request_parallel_as_completed(urls,
#                                                      page_type=page_type,
#                                                      n_conn=n_conn,
